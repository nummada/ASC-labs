...
__global__ void matrix_multiply_simple(float *ma, float *mb, float *mc, size_t width)
{
	...
	register float aux = 0;
	for(int k = 0 ; k < width ; k++) {
		aux += ma[row * width + k] * mb[k * width + col];
	}

	//TODO: write result in mc
	mc[row * width + col] = aux;
}
...
int main(void)
{
	...
	std::cout << "Time for simple implementation is: " << average_simple_time << std::endl;
	...
	std::cout << "Time for optimized implementation is: " << average_optimized_time << std::endl;
